...
export function renderTask(task: TaskDoc): string {
  ...
  const whenLine = `📅 ${task?.dueAt ? humanDateFromParts(
    (new Date(task.dueAt)).toISOString().slice(0, 10),
    `${String(new Date(task.dueAt).getHours()).padStart(2,'0')}:${String(new Date(task.dueAt).getMinutes()).padStart(2,'0')}`
  ) : '—'}`;

  const reminderLine = (() => {
    const anyt: any = task as any;
    if (anyt?.reminderAt) {
      const r = new Date(anyt.reminderAt);
...
export function renderDraft(draft: {
  ...
  const reminderLabel =
    draft.reminderPreset === 'custom'
      ? (draft.reminderDate || draft.reminderTime ? `${draft.reminderDate ?? ''} ${draft.reminderTime ?? ''}`.trim() : 'Своя дата/время')
  ...
  const whenStr = draft.dueDate || draft.dueTime ? humanDateFromParts(draft.dueDate ?? null, draft.dueTime ?? null) : '—';

  const origTitle = original ? composeTitle(original.type, original.title || '') : null;
  const origDueDate = original && original.dueAt ? new Date(original.dueAt).toISOString().slice(0,10) : null;
  const origDueTime = original && original.dueAt ? `${String(new Date(original.dueAt).getHours()).padStart(2,'0')}:${String(new Date(original.dueAt).getMinutes()).padStart(2,'0')}` : null;
  const origReminderPreset = original ? (original.reminderAt ? 'custom' : (original.reminderPreset || 'none')) : null;
  const origReminderDate = original && original.reminderAt ? new Date(original.reminderAt).toISOString().slice(0,10) : null;
  const origReminderTime = original && original.reminderAt ? `${String(new Date(original.reminderAt).getHours()).padStart(2,'0')}:${String(new Date(original.reminderAt).getMinutes()).padStart(2,'0')}` : null;
